...
class GaragePage:
    ...
    def profile_button_(self):
        ...
        try:
            element = WebDriverWait(self.driver, timeout=10).until(EC.element_to_be_clickable(PROFILE_BUTTON))
            return element
    ...
    def profile_reference(self):
            try:
                element = WebDriverWait(self.driver, timeout=5).until(EC.element_to_be_clickable(PROFILE_REFERENCE))
                return element
            except TimeoutException:
                raise NoSuchElementException("За даний час елемент не зявився на сторінці")
    ...
    def profile_text(self):
        try:
            element = WebDriverWait(self.driver, timeout=10).until(EC.presence_of_element_located(PROFILE_TEXT_CSS))
